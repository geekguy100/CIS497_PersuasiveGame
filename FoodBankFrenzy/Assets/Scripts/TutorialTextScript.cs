...
public class TutorialTextScript : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (PickupScript.objectInHand == null && tutorialsCompleted[0] == true && PickupScript.isHeld == false)
        {
            tutorialText.SetText("Controls:\nPick up a can then right click to reserve a can for later use.");
            tutorialsCompleted[1] = true;
        }
        if (PickupScript.objectInHand == null && tutorialsCompleted[1] && PickupScript.isHeld == true)
        {
            tutorialText.SetText("Controls:\nLeft click on the reserve can and take it out of reserve.");
